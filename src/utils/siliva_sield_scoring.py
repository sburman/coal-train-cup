from coal_train_cup.services.nrl_api_service import (
    __load_fixtures_from_nrl_api,
    __get_match_snapshot,
)
...
def get_round_team_winners(round: int, season: int = CURRENT_SEASON) -> list[str]:
    ...
    for game_id in game_ids:
        ...
        home_score = snapshot["gameStats"]["teams"]["teamsMatch"][0].get(
            "teamFinalScore", 0
        )
        away_score = snapshot["gameStats"]["teams"]["teamsMatch"][1].get(
            "teamFinalScore", 0
        )
        winner = (
            snapshot["gameStats"]["teams"]["teamsMatch"][0]["teamName"]
            if home_score > away_score
            else snapshot["gameStats"]["teams"]["teamsMatch"][1]["teamName"]
        )
...
if __name__ == "__main__":
    ...
    df_filtered = df_filtered.drop_duplicates(
        subset=["email", "season", "round", "team", "tryscorer"], keep="last"
    )
