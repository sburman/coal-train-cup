...
def section_admin() -> None:
    ...
    if violations:
        ...
        for violation in violations:
            st.write(f"- {violation['user']} tipped {violation['team']} {violation['count']} times")
    else:
    ...
    for user_email, venue_counts in user_venue_counts.items():
        if venue_counts['home'] > 13:
            venue_violations.append({
                'user': user_email,
                'type': 'home',
                'count': venue_counts['home']
            })
        if venue_counts['away'] > 13:
            venue_violations.append({
                'user': user_email,
                'type': 'away',
                'count': venue_counts['away']
            })
    
    if venue_violations:
        st.error(f"Found {len(venue_violations)} violations of the home/away team limit:")
        for violation in venue_violations:
            st.write(f"- {violation['user']} has {violation['count']} {violation['type']} tips (max 13)")
    else:
